...
const EmailCard = ({
  ...
  const displayDate = isToday ? format(formattedDate, 'h:mm a') : formattedDate.toDateString() === new Date(Date.now() - 86400000).toDateString() ? 'Yesterday' : format(formattedDate, 'MMM d');
  ...
  return <Card className={cn("mb-4 email-card border-l-4 transition-all overflow-hidden", isOpen ? "border-l-primary" : "border-l-transparent")} onClick={() => onOpen && onOpen(email.id)}>
      <CardContent className="p-4">
        <div className="flex items-center mb-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-full overflow-hidden mr-3">
            <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(email.sender_name)}&background=random`} alt={email.sender_name} className="w-full h-full object-cover" />
          </div>
        ...
        <div className={cn("email-body transition-all overflow-hidden", isOpen ? "max-h-full" : "max-h-16")}>
          ...
          {isOpen && email.langbase_reply && !replySubmitted && <div className="mt-6 p-4 rounded-lg border border-border dark:bg-neutral-800">
              ...
              {!isValidated && !isReplying && <div className="flex gap-2 mt-4">
                  <Button size="sm" onClick={handleValidate} className="gap-1 text-slate-50 bg-violet-600 hover:bg-violet-500">
          ...
          {isValidated && !replySubmitted && <div className="mt-4">
              <Button className="gap-1 w-full" onClick={handleSendReply}>
                <Send className="h-4 w-4" />
                Send Reply
              </Button>
            </div>}
          
          {replySubmitted && <div className="mt-6 p-4 rounded-lg border border-border bg-background">
              <div className="flex items-center mb-2">
